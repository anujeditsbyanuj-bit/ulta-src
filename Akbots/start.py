...
FEATURES_TXT = (
    ...
    f"/zip with AES-256 password, /unzip any format • Twitter/Reddit/Pinterest gallery downloads</blockquote>\n\n"
    f"<blockquote expandable><b>🎬 Media Editing</b>\n"
    f"Format conversion, audio extraction, watermark, metadata, screenshots, sample clips, spoiler tags, custom thumbnails/captions, auto-rename engine</blockquote>\n\n"
...
ALL_COMMANDS = [
    ...
    ('zippass', '🔒 Password-protect the zip (AES-256)'),
]
CMDS_PER_PAGE = 20
...
def _cmdlist_buttons(page: int) -> InlineKeyboardMarkup:
    ...
    nav_row = []
    if page > 0:
        nav_row.append(make_button(" ◀️ ᴘʀᴇᴠ ", callback_data=f"cmdlist:{page - 1}"))
    if page < total_pages - 1:
        nav_row.append(make_button(" ɴᴇxᴛ ▶️ ", callback_data=f"cmdlist:{page + 1}"))
    rows = []
    if nav_row:
        rows.append(nav_row)
    rows.append([
        make_button(" ⬅️ ʙᴀᴄᴋ ", callback_data="menu_msg_back", icon_custom_emoji_id=ICON_HOME),
        make_button(" ❌ ᴄʟᴏsᴇ ", callback_data="menu_msg_close", icon_custom_emoji_id=ICON_CLOSE),
    ])
...
def get_start_buttons():
    if BUTTON_STYLE_SUPPORTED:
        ...
        return InlineKeyboardMarkup([
            ...
            [
                make_button(" 🚀 ʙᴏᴛ ғᴇᴀᴛᴜʀᴇs ", callback_data="features_btn", icon_custom_emoji_id=ICON_LIST),
            ],
            [
                make_button(" 📜 ᴀʟʟ ᴄᴏᴍᴍᴀɴᴅs ", callback_data="cmdlist:0",    icon_custom_emoji_id=ICON_LIST),
            ],
            [
